<script lang="ts">
  ...
  let componentId = `a${(Math.random() + 1).toString(36).substring(7)}`;
</script>
...
<Popover triggeredBy="#{componentId}" class="w-72 text-sm font-light text-gray-500 bg-white dark:bg-gray-800 dark:border-gray-600 dark:text-gray-400" placement="bottom-start">
  <div class="p-3 space-y-2 normal-case">
      {text}
  </div>
</Popover>
